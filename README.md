...
Unity用のプロジェクトをGitHubに登録する手順は、以下を参照のこと。
- http://am1tanaka.hatenablog.com/entry/2016/02/05/102508

## 課題3
課題2で作成したUnityのプロジェクトをGitHub Pages上で公開する。公開手順は以下を参照のこと。
- http://am1tanaka.hatenablog.com/entry/2015/11/27/115926

1. 公開するユーザーページのトップ(index.html)を用意する
2. 誰のページか分かるように学籍番号を表示させること
3. 作成したチュートリアルのページについて、以下を記載する
  - ライセンス表示
    - チュートリアルが掲載されているWebサイトに、著作権やライセンスの注意書きがあれば、それに従った表記を行う
    - 特になければ、以下のものを記載する
      - 著作権者の氏名
        - Webページの最後などにCopyrightや(C)に続けて書かれている名前
        - 見当たらない場合は作者のハンドルネームらしきものか、団体の名称
        - それもわからない場合はページタイトル
  - Webページのタイトル
    - Webページの最初に表示されていたり、ブラウザのタイトルバーに表示されるページのタイトル
  - チュートリアルページへのリンク
  - 作品名
  - 「以上のチュートリアルを参照して作成いたしました。」などの文言
  - 改造していたら自分が改造したポイントを箇条書きなどで簡潔に記載
  - 簡単な操作方法
  - 以上を、トップページであるindex.htmlと、Unityの動作しているページの双方に記載しておくとよい
...
## 提出方法
- GitHubの自分のアカウントにコミットしてPublish/Syncする

## 注意点
- 基本的に、自力で進めること
- 作業が引っかかったら質問して、ヒントを得ること（原則として、直接、操作方法や解決策は提示しない）
- 課題２を開発するためのUnityのプロジェクトを作成したら、すぐにGitHubに登録する
- その際に、操作方法が分からないところがあったら質問する（ただし、質問には手順を直接教えることはしない。読むべき資料の場所を提示する）
...
## 講義資料
- 12回目：[オブジェクト指向の演習](https://github.com/tanakaedu/obj-prac)
